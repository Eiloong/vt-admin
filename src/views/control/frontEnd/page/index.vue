...
export default defineComponent({
  setup() {
    ...
    const getAuthBtnList = computed(() => {
      return userStore.userInfo.authBtnList
    })
    // 初始化用户权限
    const initUserAuth = () => {
      state.userAuth = userStore.userInfo.roles[0]
    }
    // 用户权限改变时
    const onRadioChange = async () => {
      ...
      const userInfos = {
        userId: state.userAuth === 'admin' ? 0 : 1,
      ...
      userStore.setUserInfo(userInfos) // 请注意执行顺序(存储用户信息vuex)
      await setAddRoute()
